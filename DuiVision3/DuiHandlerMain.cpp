...
LRESULT CDuiHandlerMain::OnDuiBtnSystemStart(UINT uID, CString strName, UINT Msg, WPARAM wParam, LPARAM lParam){
	...
	if(m.Compare(L"未开启")==0){
		pTabCtrl_F->SetControlTitle(L"已开启");
		//pTabCtrl_F->SetImage(L"IDB_BT_ICON");
		((CDuiTabCtrl*)(pDlg_m->GetControl(_T("button.start"))))->SetControlDisable(FALSE);
		((CDuiTabCtrl*)(pDlg_m->GetControl(_T("button.end"))))->SetControlDisable(FALSE);
		((CDuiTabCtrl*)(pDlg_m->GetControl(_T("button.newInformation"))))->SetControlDisable(FALSE);
		
		//开启逻辑
		//假设当前为R3
		((CDuiTabCtrl*)(pDlg_m->GetControl(_T("system.r3"))))->SetControlVisible(TRUE);
	}else{
		pTabCtrl_F->SetControlTitle(L"未开启");
		//pTabCtrl_F->RemoveControl(pTabCtrl_F);
		//关闭逻辑
		//取消所有信息显示
		((CDuiTabCtrl*)(pDlg_m->GetControl(_T("system.r3"))))->SetControlVisible(FALSE);
		((CDuiTabCtrl*)(pDlg_m->GetControl(_T("system.r0"))))->SetControlVisible(FALSE);
		((CDuiTabCtrl*)(pDlg_m->GetControl(_T("system.vt"))))->SetControlVisible(FALSE);

		((CDuiTabCtrl*)(pDlg_m->GetControl(_T("button.start"))))->SetControlDisable(TRUE);
		((CDuiTabCtrl*)(pDlg_m->GetControl(_T("button.end"))))->SetControlDisable(TRUE);
